...
class WaveGANGenerator(nn.Module):
    ...
    def forward(self, audio, z):
        ...
        # audio_after_conv = self.audio_conv(audio)        # Shape: [batch_size, self.audio_conv_last_dim, audio_dim//64]
        audio_after_conv = audio.view(-1, self.noise_chan, self.audio_af_conv_dim) # 32 - 4096
        # print('print audio', audio.shape)
        # print(z.shape)
        # print(audio.shape)
        # x = torch.stack((audio_after_conv, z), dim=1)        # Shape: [batch_size, self.audio_conv_last_dim, self.noise_chan, audio_af_conv_dim]
        x = audio_after_conv * 0.7 + z * 0.3 # combine noise and processed audio
        # print('GEN')
        # print(x.shape)
        # x = x.view(-1, self.audio_conv_last_dim + self.noise_chan, self.audio_af_conv_dim)
        x = x.view(-1, self.audio_af_conv_dim, self.noise_chan)
        # print(x.shape)
        output = self.comb_conv(x)
        # print(output.shape)
        # output = audio + output * 0.7
        return output
...
class WaveGANDiscriminator(nn.Module):
    def __init__(self, audio_dim=131072, num_channels=1, 
        ...
        self.conv = nn.Sequential(
            ...
            # nn.Conv1d(in_channels=model_size*4, out_channels=model_size*8, kernel_size=25, stride=4, padding=11),  # 2048 -> 512
            # PhaseShuffle(shift_factor),
            # nn.LeakyReLU(0.2),

            # nn.Conv1d(in_channels=model_size*8, out_channels=model_size*16, kernel_size=25, stride=4, padding=11),  # 2048 -> 512
            # PhaseShuffle(shift_factor),
            # # nn.BatchNorm1d(128),
            # nn.LeakyReLU(0.2),
